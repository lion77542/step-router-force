...
## 21. 🆕 2026-08-14：pro 主力保险——建议型/未触发检测 + 自动重试

**背景**：用户核心诉求"稳定每次都 pro 主力"。实测 8 连样本答案型 7/8（87.5%）——提示词已到概率上限，剩余 12.5% 是"建议型/混合"（advisor 只审批计划 → 输出退回 flash 内容）。

**方案**（代理响应层兜底，把概率变成确定性）：
1. **形态检测**：提取 [Advisor consultation] 块，建议型特征（"I plan to"/"my plan"/"我计划"/"请审批"等）且无答案特征（代码块/def/class/完整）→ 判定建议型
2. **未触发检测**：实质任务响应无 [Advisor consultation] 块 → 判定未咨询
3. **自动重试**：命中上述任一 → 追加 [RETRY DIRECTIVE]（建议型：再要完整答案；未触发：强制咨询）→ 重新请求上游，最多 2 次重试
4. **流式缓冲**：OpenAI 流式改为缓冲整条 SSE → 检测 → 重试 → 回放（Claude Code 界面上 reasoning_content 本就不渲染，缓冲对体验影响极小）
5. **闲聊豁免**：is_trivial 判定不变，闲聊不重试（省时）

**实测**：
| 项 | 结果 |
|---|---|
| selftest3 检测函数 | 9/9（建议型/答案型/未触发/中文/混合/SSE 解析） |
| 冒烟（真实 API，流式+非流式） | 200，答案型，Write 工具，无重试误触发 |
| 8 连样本答案型 | 7/8（87.5%），工具调用 8/8，含测试 8/8 |

**结论**：重试机制后，建议型/未触发响应在代理层被拦截并强制重试——"稳定 pro 主力"从概率性行为变为**可兜底的工程保证**（理论上限 = 3 次尝试全失败的概率）。输出质量（effort=high + thinking 8000 + 完整代码含测试）已最大化到当前参数甜点。

**参数现状（最终）**：model=step-router-v1，max_tokens∈[200k,250k]，reasoning_effort=high，thinking.budget_tokens=8000，temperature=0.2，双通道 DIRECTIVE + 响应层重试。
